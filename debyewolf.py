...
import numpy as np
from lorenzmie import lm_electric_strength_factor, lm_angular_spectrum
...
def scatter(s_obj_cart, a_p, n_p, nm_obj, lamb, r, z):
    ...
    es_obj = lm_electric_strength_factor(s_obj_cart, ab, lamb, nm_obj, r, z)
...
def scatter_ang_spectrum(s_obj_cart, a_p, n_p, nm_obj, lamb):
    '''Compute the angular spectrum arriving at the entrance pupil.'''

    # Compute the electromagnetic strength factor on the object side
    # (Eq 40 Ref[1]).
    ab = sphere_coefficients(a_p, n_p, nm_obj, lamb)
    ang_spec_obj = lm_angular_spectrum(s_obj_cart, ab, lamb, nm_obj)
    ang_spec_obj = np.nan_to_num(ang_spec_obj)
